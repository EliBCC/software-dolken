...
$(BUILD_DIR)/Debug/stamp: CMakeLists.txt scripts/toolchain.cmake
	...
	@cmake -S . -B $(BUILD_DIR)/Debug $(CMAKE_FLAGS) -DCMAKE_BUILD_TYPE=Debug -G "$(CMAKE_GENERATOR)" ../..

$(BUILD_DIR)/Release/stamp: CMakeLists.txt scripts/toolchain.cmake
	@cmake -E make_directory $(BUILD_DIR)/Release
	@cmake -S . -B $(BUILD_DIR)/Release $(CMAKE_FLAGS) -DCMAKE_BUILD_TYPE=Release -G "$(CMAKE_GENERATOR)" ../..

build: $(BUILD_DIR)/$(BUILD_TYPE)/stamp
	@cmake --build $(BUILD_DIR)/$(BUILD_TYPE) --parallel $J

build-%: $(BUILD_DIR)/$(BUILD_TYPE)/stamp
	@cmake --build $(BUILD_DIR)/$(BUILD_TYPE) --target $* --parallel $J

upload-%: $(BUILD_DIR)/$(BUILD_TYPE)/stamp
	@cmake --build $(BUILD_DIR)/$(BUILD_TYPE) --target upload-$* --parallel $J
